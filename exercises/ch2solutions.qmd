---
title: "Solutions to Exercises - Chapter 2"
...
### Exercise 2.1
Let $X_1,\ldots,X_n \vert \theta \overset{\mathrm{iid}}{\sim} \mathrm{Expon}(\theta)$ be exponentially distributed data. Show that the Gamma distribution is the conjugate prior for this model.
...
\begin{align}
 ...
 & = \theta^n e^{-\theta\sum_{i=1}^n x_i}\theta^{\alpha-1}e^{-\beta\theta}  \\
...
\emph{Hint}: The posterior is no longer tractable due to contributions of the censored patients to the likelihood. For the censored patients we only know that they lived \emph{at least} the number of days recorded in the dataset. The likelihood contribution $p(x_c \vert \theta)$ for the $c$th censored patient with recorded time $x_c$ is therefore $p(X \geq x_c \vert \theta) = e^{-\theta x_c}$, which follows from the distribution function of the exponential distribution $p(X \leq x \vert \theta) = 1 - e^{-\theta x}$.
